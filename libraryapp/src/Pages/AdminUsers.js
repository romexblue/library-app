...
const AdminUsers = () => {
    ...
    const fetchUsers = async (page) => {
        await axios
            .get(`${process.env.REACT_APP_API_URL}/auth/all/${page}`, {
                headers: {
    ...
    const handleDeleteConfirm = () => {
        axios
            .delete(`${process.env.REACT_APP_API_URL}/auth/${userData.id}`, {
                headers: {
                    accessToken: sessionStorage.getItem("accessToken"),
                    userId: sessionStorage.getItem("id"),
                },
                            ...
                            users.map((userObj, index) => {
                                return (
                                    <tr key={index}>
                                        ...
                                        <td>{userObj?.type}</td>
                                        <td>
                                            <div className={us.passMaker}>
                                                <input
                                                    className={us.secInput4}
                                                    type="password"
                                        ...
                                        <td>
                                            <button
                                                className={us.editButton}
                                                onClick={() =>
                                                    handleClick(userObj, "Edit")
                                                }
                                            >
                                                <img src={image1} alt=""></img>
                                            </button>
                                            <button
                                                className={us.deleteButton}
                                                onClick={() =>
                                                    handleClick(
                                                        userObj,
                                                        "Delete"
