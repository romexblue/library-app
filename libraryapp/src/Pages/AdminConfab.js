...
const AdminConfab = () => {
    ...
    const handleDeleteConfirm = () => {
        axios
            .delete(
                `${process.env.REACT_APP_API_URL}/confab/${confabData.id}`,
                {
                    headers: {
                        accessToken: sessionStorage.getItem("accessToken"),
                        userId: sessionStorage.getItem("id"),
    ...
    return (
        <div className={con.confabPage}>
            <div className={con.pageHeader}>
                ...
                <div className={con.section2}></div>
                <div className={con.section3}>
                    <div className={con.button1}>
                        <button
                            className={con.addBtn}
                            onClick={() => handleClick([], "Add")}
                        >
                            <div className={con.plusSign}>+</div>
                            <p>Add Space</p>
                        </button>
                    </div>
                </div>
                    ...
                    <tbody className={con.tableContent}>
                        {confabs.map((confab, index) => (
                            <tr key={confab.id}>
                                ...
                                <td>
                                    <button
                                        className={con.editButton}
                                        onClick={() =>
                                            handleClick(confab, "Edit")
                                        }
                                    >
