...
const AdminReservation = () => {
    ...
    const getReservationByFilter = (confId, status, date, page) => {
        axios
            .get(
                ...
                }/reservation/requests/${confId}/${status}/${date
                    .toISOString()
                    .slice(0, 10)}/${page}`,
                {
                    headers: {
    ...
    const getReservationById = (event) => {
        ...
        axios
            .get(
                `${process.env.REACT_APP_API_URL}/reservation/requests/find-by/${value}`,
                {
                    headers: {
                        accessToken: sessionStorage.getItem("accessToken"),
                        userId: sessionStorage.getItem("id"),
    ...
    const changeStatus = async (res, status) => {
        ...
        await axios
            .patch(
                `${process.env.REACT_APP_API_URL}/reservation/requests/find-by/${res}`,
                data,
                {
                    headers: {
                        accessToken: sessionStorage.getItem("accessToken"),
        ...
        <div>
            <div className={res.pageTop} id="">
                ...
                <div className={res.pageFilter2} id="j">
                    <select
                        className={res.selectInput}
                        disabled={searching}
                        id="confab-select"
                        onChange={handleConfabSelectChange}
                    >
                        {confabData.map((confab) => (
                            <option
                                className={res.optionSelect}
                                key={confab.id}
                                value={confab.id}
                            >
                                {confab.name}
                ...
                <div className={res.pageFilter4} id="">
                    <form
                        onSubmit={(event) => getReservationById(event)}
                        className={res.searchForm}
                    >
                        <input
                            className={res.searchInput}
                            type="number"
                            placeholder="Find by ID"
                            onChange={handleInputChange}
                            value={value}
                        ></input>
                        <button className={res.searchBtnBox} type="submit">
                            <img
                                className={res.searchBtn}
                                src={image1}
                                alt=""
                            ></img>
                        </button>
